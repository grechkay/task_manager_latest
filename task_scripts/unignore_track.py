...
def main(trackname):
	...
	if trackname + '.track' not in all_track_targets:
		...
		if trackname not in ignored_tracks:
			print(bcolors.FAIL + "\n\tTrack target was not ignored. Exiting.\n" + bcolors.ENDC)
			return

		else:
			ignored_tracks.remove(trackname)

		# and finally save it
		with open(ignore.ignore_path, 'wb') as f:
			pk.dump(ignored_tracks, f)

		print(bcolors.BOLD + bcolors.OKBLUE + "\n\t" + trackname + " has officially been unignored. \n\tYou can resume tracking this track.\n" + bcolors.ENDC)
...
if __name__ == '__main__':
	...
	parser.add_argument("trackname", help="The name of the track target that you wish to now unignore from all future statistics and showing")
